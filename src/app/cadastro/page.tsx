...
export default function CadastroPage() {
  ...
  const { isValid, email, isLoading: isTokenLoading, error: tokenError } = useValidateToken(token);
  const { registerUser, isLoading: isRegistering, isSuccess, error: registerError } = useRegisterUser();
  ...
  useEffect(() => {
    if (!isTokenLoading) {
      if (!isValid || tokenError) {
        router.push('/acesso-negado');
      }
    }
  }, [isTokenLoading, isValid, tokenError, router]);
            ...
            <form className="flex flex-col gap-4" onSubmit={handleSubmit}>
              ...
              <div className="pt-4">
                ...
                <button
                  ...
                  className="flex h-14 w-full items-center justify-center rounded-lg bg-[#135bec] px-6 text-base font-semibold text-white transition-colors hover:bg-[#135bec]/90 focus:outline-none focus:ring-2 focus:ring-[#135bec] focus:ring-offset-2 disabled:opacity-50 dark:focus:ring-offset-[#101622]"
                >
                  {isRegistering ? "Registrando..." : "Finalizar Cadastro e Acessar"}
                </button>
              </div>
            </form>
          )}
